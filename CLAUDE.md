...
## 그림을 다룰 때
  ...
  · **깨진 교체본이 멀쩡한 번들 그림을 가리지 않는다.** 교체본 적재에 실패하면
    번들로 내려간다.
  · **'이미 받아 뒀나'를 `glob` 으로 묻지 않는다.** `folder.glob(f'{stem}.*')` 는
    다른 스레드가 쓰는 중인 `<stem>.png.<uuid>.part` 까지 잡는다. 그걸 표지라고
    돌려주면 곧 rename 으로 사라질 경로가 DB 에 박힌다. 확장자를 하나씩 짚어
    본다(`covers._existing_cover()`). 60회에 2회 나오는 경합이었다.
...
10. **SQLite 연결은 명시적으로 닫는다. Windows 는 GC 를 기다려 주지 않는다.**
    Linux 는 참조 계수가 즉시 거둬 파일이 풀리지만, Windows 에서는 열려 있는
    파일을 **지우지도 옮기지도 못한다**. 손상된 기록 파일을 백업으로 되돌릴 수
    없게 된다는 뜻이다. `_new_connection()` 은 설정(PRAGMA) 도중 터지면 연결을
    닫고 나간다 — 손상된 파일에서 `journal_mode=WAL` 이 실제로 터진다.
    **이 차이는 Linux 에서 절대 안 보인다.** CI 의 Windows 잡이 잡아냈다.
11. **한글을 `print()` 하다 죽을 수 있다.** 영문 Windows 콘솔은 cp1252 라
    한글을 못 담는다. 안내문 한 줄에 `UnicodeEncodeError` 로 도구가 통째로
    죽는다. 한국어 Windows(cp949)에서는 안 보인다. `main()` 과 `tools/*` 가
    시작할 때 stdout·stderr 를 UTF-8 로 다시 연다. 테스트 탐침이 찍는 표식은
    아예 ASCII 로 둔다.
...
**Windows 에서 돌려보기 전에는 모르는 것이 있다.** 감사 뒤 `.github/workflows/`
에 Windows 잡을 두고 처음 돌리자, 438건 중 4건이 걸렸다 — 그중 둘은 진짜 결함
(손상 파일을 연 뒤 연결이 안 닫혀 파일이 잠김, 한글 출력이 cp1252 에서 죽음)
이었고 **둘 다 Linux 에서는 원리상 재현되지 않는다.** 그 자리가 이 코드가
Windows 에서 도는 유일한 자리다. 빌드 결과는 `dist/` 가 아니라 **실제로 exe 를
띄워 25초 살아 있는지, 기록 파일이 생기는지**로 확인한다.

**어쩌다 한 번 깨지는 것을 넘기지 않는다.** 전체 테스트를 반복해 돌리다 한 번
실패한 적이 있는데, 다음 170회가 통과했다. 넘어가는 대신 흔들릴 만한 테스트를
따로 60회씩 돌려 범인을 찾았다 — 표지 동시 다운로드가 **받는 중인 `.part` 를
표지라고 돌려주는** 실제 결함이었다(3%). 경합을 재현하는 테스트는 그 자체로
또 흔들리므로, 상황만 만들어 두고 확정적으로 확인하는 테스트로 못 박았다.
